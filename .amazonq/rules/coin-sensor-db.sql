CREATE TABLE `detected_coins` (
	...
	`dislike_count`	BIGINT	NOT NULL	DEFAULT 0	COMMENT '싫어요 수'
);
...
CREATE TABLE `ohlcvs` (
	...
	`close_time`	DATETIME	NOT NULL	COMMENT '마감 시간',
...
CREATE TABLE `exchange_coins` (
	...
	`enable_detection`	TINYINT(1)	NOT NULL	DEFAULT 1	COMMENT '탐지 활성화 여부'
);
...
CREATE TABLE `coin histories` (
	`coin_history_id`	BIGINT	NOT NULL	DEFAULT auto_increment	COMMENT '코인 기록  ID',
	`coin_id`	BIGINT	NOT NULL	DEFAULT auto_increment	COMMENT '코인 ID',
	`value`	BIGINT	NOT NULL	COMMENT '값',
	`type`	VARCHAR(255)	NOT NULL	COMMENT '타입',
	`created_at`	DAT ETIME	NOT NULL	COMMENT '생성 시간'
);
...
ALTER TABLE `coin histories` ADD CONSTRAINT `PK_COIN HISTORIES` PRIMARY KEY (
	`coin_history_id`
);
...
ALTER TABLE `coin histories` ADD CONSTRAINT `FK_coins_TO_coin histories_1` FOREIGN KEY (
	`coin_id`
)
REFERENCES `coins` (
	`coin_id`
